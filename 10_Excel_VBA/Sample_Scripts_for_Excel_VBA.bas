...
Sub variousScripts()
    ...
    ' Find the LAST ROW in a column full of data
    
    Dim LastRow As String
    LastRow = Cells(Rows.Count, "A").End(xlUp).Row
    
    'for multiple spreadsheets tabs, use LastRow=ws.Cells(Rows.Count, "A").End(x1Up).Row instead of the above one
        ' NOTE:  letter after Rows.Count, is the COLUMN you want to count
...
' May need to first:  Dim ws as Worksheet
For Each ws In Worksheets
    'do something here
Next ws


'Split a STRING apart into pieces based on a SPLICE-POINT or character to split on
...
Range("A1").Value = split_string(0) 'print "Hello" to Cell A1
Range("B1").Value = split_string(1) 'print "World" to Cell B1
